...
class BaseCAM:
    ...
    def get_cam_image(self, input_tensor, target_layer, target_category,
        activations, grads, eigen_smooth=False):
    ...
    def forward(self, input_tensor, target_category=None, eigen_smooth=False):
        ...
        if self.compute_input_gradient:
            input_tensor = paddle.to_tensor(input_tensor, requires_grad=True)
    ...
    def compute_cam_per_layer(self, input_tensor, target_category, eigen_smooth
        ...
        activations_list = [a.cpu().data.numpy() for a in self.
            activations_and_grads.activations]
        grads_list = [g.cpu().data.numpy() for g in self.
            activations_and_grads.gradients]
        target_size = self.get_target_width_height(input_tensor)
        cam_per_target_layer = []
        for target_layer, layer_activations, layer_grads in zip(self.
            target_layers, activations_list, grads_list):
            cam = self.get_cam_image(input_tensor, target_layer,
                target_category, layer_activations, layer_grads, eigen_smooth)
    ...
    def forward_augmentation_smoothing(self, input_tensor, target_category=\
        None, eigen_smooth=False):
        transforms = tta.Compose([tta.HorizontalFlip(), tta.Multiply(
            factors=[0.9, 1, 1.1])])
        cams = []
    ...
    def __call__(self, input_tensor, target_category=None, aug_smooth=False,
        eigen_smooth=False):
