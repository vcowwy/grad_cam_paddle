...
class FullGrad(BaseCAM):
    ...
    def __init__(self, model, target_layers, use_cuda=False,
        ...
        def layer_with_2D_bias(layer):
            ...
            if type(layer) in bias_target_layers and layer.bias is not None:
                return True
            return False
        target_layers = find_layer_predicate_recursive(model,
            layer_with_2D_bias)
        super(FullGrad, self).__init__(model, target_layers, use_cuda,
            reshape_transform, compute_input_gradient=True)
        self.bias_data = [self.get_bias_data(layer).cpu().numpy() for layer in
            target_layers]

    def get_bias_data(self, layer):
        if isinstance(layer, paddle.nn.BatchNorm2D):
            bias = -(layer.running_mean * layer.weight / paddle.sqrt(layer.
                running_var + layer.eps)) + layer.bias
            return bias.data
        else:
    ...
    def scale_accross_batch_and_channels(self, tensor, target_size):
        ...
        reshaped_tensor = tensor.reshape(batch_size * channel_size, *tensor
            .shape[2:])
        result = self.scale_cam_image(reshaped_tensor, target_size)
        result = result.reshape(batch_size, channel_size, target_size[1],
            target_size[0])
        return result
    ...
    def compute_cam_per_layer(self, input_tensor, target_category, eigen_smooth
        ...
        for bias, grads in zip(self.bias_data, grads_list):
            ...
            result = self.scale_accross_batch_and_channels(bias_grad,
                target_size)
        ...
        if eigen_smooth:
            cam_per_target_layer = self.scale_accross_batch_and_channels(
                cam_per_target_layer, (target_size[0] // 8, target_size[1] //
                8))
            ...
            cam_per_target_layer = np.sum(cam_per_target_layer, axis=1)[:,
                None, :]
